...
public class TakingTurnsQueue
{
    ...
    public Person GetNextPerson()
    {
        ...
        if (person.Turns <= 0)
        {
            // Infinite turns, do not decrement turns, re-enqueue as is
            _people.Enqueue(person);
        }
        else
        {
            // positive turns, decrement first
            person.Turns -= 1;
            if (person.Turns > 0)
            {
                // still have turns left, re-enqueue
                _people.Enqueue(person);
            }
            // else no turns left, do not re-enqueue (person removed)
        }
